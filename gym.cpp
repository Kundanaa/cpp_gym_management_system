...
public:
	...
	void create_mem()
	{
		...
		if(k==1)
		{
		t1.open("c://trainer1.txt",ios::app);
		t1<<mem_name<<endl;
		}
		ofstream t2;
		if (k==2){
			t2.open("c://trainer2.txt",ios::app);
			t2<<mem_name<<endl;
		}
		switch (k)
		{
...
void save_member()
{
	fp.open("c://list_of_members.txt", ios::app);
	m1.create_mem();
...
void show_all()
{
	...
	cout << endl
		 << "\t\t...RECORDS...\n";
	fp.open("c://list_of_members.txt", ios::in);
...
void edit_member()
{
	...
	fp.open("c://list_of_members.txt", ios::in | ios::out);
...
void delete_member()
{
	...
	fp2.close();
	fp.close();
	remove("c://list_of_members.txt");
	rename("c://Temp.txt", "c://list_of_members.txt");
	cout << endl
		 << endl
		 << "\tRecord Deleted...\n\n";
	getchar();
